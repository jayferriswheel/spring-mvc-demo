...
import com.carto.juc.ReentrantLock;
...
public class ConcurrentHashMap<K,V> extends AbstractMap<K,V>
    ...
    private final Node<K,V>[] initTable() {
        ...
        while ((tab = table) == null || tab.length == 0) {
            if ((sc = sizeCtl) < 0)
                Thread.yield(); // lost initialization race; just spin 就是暗示可以释放cpu使用权，管不管用先不理
            else if (U.compareAndSwapInt(this, SIZECTL, sc, -1)) { // 又一处CAS铁证
                try {
        ...
        if (tab != null && (f instanceof ForwardingNode) &&
            ...
            while (nextTab == nextTable && table == tab &&
                ...
                if ((sc >>> RESIZE_STAMP_SHIFT) != rs || sc == rs + 1 ||
                    ...
                    break;
                // 经过一系列乱七八糟的判断，改变下状态，加1个线程，表示我加入来扩容了。
                if (U.compareAndSwapInt(this, SIZECTL, sc, sc + 1)) {
    ...
    private final void transfer(Node<K,V>[] tab, Node<K,V>[] nextTab) {
        ...
        // 计算每个线程大概处理的桶的个数，兜底是16
        if ((stride = (NCPU > 1) ? (n >>> 3) / NCPU : n) < MIN_TRANSFER_STRIDE)
            stride = MIN_TRANSFER_STRIDE; // subdivide range
        if (nextTab == null) {            // initiating
            try {
                @SuppressWarnings("unchecked")
                Node<K,V>[] nt = (Node<K,V>[])new Node<?,?>[n << 1]; // 创建了一个2倍数组
                nextTab = nt;
        ...
        } // 完成新数组nextTab初始化，用于扩容
        int nextn = nextTab.length;
        ForwardingNode<K,V> fwd = new ForwardingNode<K,V>(nextTab);
        boolean advance = true; // 判断是否继续处理，即推进
        ...
        for (int i = 0, bound = 0;;) { // 死循环开干
                ...
                // 这里会cas切换transferIndex，会减去stride，这样每一个线程就会分配其中一部分
                        ...
                        (this, TRANSFERINDEX, nextIndex,
                                nextBound = (nextIndex > stride ?
                                        nextIndex - stride : 0))) {
                    bound = nextBound; // 这个值是当前线程可以处理的区间的最小下标
                    i = nextIndex - 1; // 最大下标
            ...
            else if ((f = tabAt(tab, i)) == null) // 为null，则用fwd占位，这样别的线程就知道这个桶已经搞定了
                advance = casTabAt(tab, i, null, fwd);
            else if ((fh = f.hash) == MOVED)
                advance = true; // already processed 继续推进
            else { // 那下面就上锁开始处理数据，下面的操作类似hashmap
                synchronized (f) {
                    if (tabAt(tab, i) == f) {
                        Node<K,V> ln, hn;
                        if (fh >= 0) {
